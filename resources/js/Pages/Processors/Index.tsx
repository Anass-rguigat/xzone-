...
export default function Index({ processors }: Props) {
    ...
    const getSortIcon = (key: keyof Processor) => {
        if (!sortConfig || sortConfig.key !== key) return <ChevronUpIcon className="h-3 w-3 ml-1 opacity-50" />;
        return sortConfig.direction === 'asc'
                                ...
                                <thead className="bg-gray-50">
                                    <tr>
                                        {[
                                            ...
                                            { key: 'core_count', label: 'Cœurs', sortable: true },
                                            { key: 'socket', label: 'Socket', sortable: true },
                                            { key: 'thermal_design_power', label: 'TDP', sortable: true },
                                ...
                                <tbody className="divide-y divide-gray-200">
                                    {currentItems.map((processor) => (
                                        <tr key={processor.id} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="px-4 py-2 text-sm text-gray-900">{processor.name}</td>
                                            <td className="px-4 py-2 text-sm text-gray-600">{processor.brand.name}</td>
                                            <td className="px-4 py-2 text-sm text-blue-600">{processor.price} DH</td>
                                            <td className="px-4 py-2 text-sm text-gray-700">{processor.core_count}</td>
                                            <td className="px-4 py-2 text-sm text-gray-600 uppercase">{processor.socket}</td>
                                            <td className="px-4 py-2 text-sm text-gray-700">{processor.thermal_design_power}W</td>
