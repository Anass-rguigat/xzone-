...
export default function Index({ powerSupplies }: Props) {
    ...
    const [sortConfig, setSortConfig] = useState<{ key: keyof PowerSupply; direction: 'asc' | 'desc' } | null>(null);
    ...
    const handleDelete = (id: number) => {
        destroy(`/power-supplies/${id}`, {
            preserveScroll: true,
            onSuccess: () => toast.success('Alimentation supprimée avec succès'),
    ...
    const handleSort = (key: keyof PowerSupply) => {
    ...
    const filteredPowerSupplies = powerSupplies.filter(powerSupply =>
        ...
        powerSupply.brand.name.toLowerCase().includes(searchTerm) ||
        powerSupply.form_factor.toLowerCase().includes(searchTerm)
    );
    ...
    const currentItems = sortedPowerSupplies.slice(indexOfFirstItem, indexOfLastItem);
    const totalPages = Math.ceil(sortedPowerSupplies.length / itemsPerPage);

    const getSortIcon = (key: keyof PowerSupply) => {
                    ...
                    <div className="rounded-lg border border-gray-200 overflow-hidden shadow-sm">
                        <div className="overflow-x-auto">
                            <table className="min-w-full">
                                ...
                                <tbody className="divide-y divide-gray-200">
                                    {currentItems.map((powerSupply) => (
                                        <tr key={powerSupply.id} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="px-4 py-2 text-sm text-gray-900">{powerSupply.name}</td>
                                            <td className="px-4 py-2 text-sm text-gray-600">{powerSupply.brand.name}</td>
                                            <td className="px-4 py-2 text-sm text-blue-600">{powerSupply.price ? `${powerSupply.price} DH` : '-'}</td>
                                            <td className="px-4 py-2 text-sm text-gray-700">{powerSupply.capacity}W</td>
                                            <td className="px-4 py-2 text-sm text-gray-600">{powerSupply.efficiency}</td>
                                            <td className="px-4 py-2 text-sm text-purple-600 uppercase">{powerSupply.form_factor}</td>
                                            <td className="px-4 py-2 text-sm">
                                                <span className={`px-2 py-0.5 rounded-lg ${powerSupply.modular ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                                                    {powerSupply.modular ? 'Oui' : 'Non'}
                                                </span>
                                            </td>
                                                ...
                                                <td className="px-4 py-2">
                                                    <div className="flex gap-2">
                                                        {can(user, 'Edit_Composants') && (
                                                            <Link
                                                                href={`/power-supplies/${powerSupply.id}/edit`}
                        ...
                        <div className="flex flex-col sm:flex-row justify-between items-center p-3 border-t border-gray-200 text-xs">
                            <div className="mb-2 sm:mb-0 text-gray-600">
                                {sortedPowerSupplies.length} résultat{sortedPowerSupplies.length > 1 && 's'} • Page {currentPage} sur {totalPages}
                            </div>
