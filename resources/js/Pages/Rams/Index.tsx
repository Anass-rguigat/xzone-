...
export default function Index({ rams }: Props) {
    ...
    const [sortConfig, setSortConfig] = useState<{ key: keyof Ram; direction: 'asc' | 'desc' } | null>(null);
    ...
    const handleSort = (key: keyof Ram) => {
    ...
    const filteredRams = rams.filter(ram =>
        ...
        ram.brand.name.toLowerCase().includes(searchTerm) ||
        ram.type.toLowerCase().includes(searchTerm)
    );
    ...
    const currentItems = sortedRams.slice(indexOfFirstItem, indexOfLastItem);
    const totalPages = Math.ceil(sortedRams.length / itemsPerPage);
