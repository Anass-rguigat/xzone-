...
export default function Index({ raidControllers }: Props) {
    ...
    const [sortConfig, setSortConfig] = useState<{ key: keyof RaidController; direction: 'asc' | 'desc' } | null>(null);
    ...
    const handleDelete = (id: number) => {
        destroy(`/raid-controllers/${id}`, {
            preserveScroll: true,
            onSuccess: () => toast.success('Contrôleur RAID supprimé avec succès'),
    ...
    const handleSort = (key: keyof RaidController) => {
    ...
    const filteredControllers = raidControllers.filter(controller =>
        controller.name.toLowerCase().includes(searchTerm) ||
        controller.brand.name.toLowerCase().includes(searchTerm) ||
        controller.model.toLowerCase().includes(searchTerm)
    );

    const sortedControllers = [...filteredControllers].sort((a, b) => {
    ...
    const currentItems = sortedControllers.slice(indexOfFirstItem, indexOfLastItem);
    const totalPages = Math.ceil(sortedControllers.length / itemsPerPage);

    const getSortIcon = (key: keyof RaidController) => {
                    ...
                    <div className="rounded-xl border border-gray-200 overflow-hidden shadow-sm">
                                ...
                                <thead className="bg-gray-50">
                                    <tr>
                                        {[
                                            ...
                                            { key: 'supported_levels', label: 'Niveaux RAID', sortable: true },
                                            ...
                                            <th 
                                                ...
                                                onClick={() => header.sortable && handleSort(header.key as keyof RaidController)}
                                            >
                                                <div className="flex items-center group">
                                                    {header.label}
                                                    {header.sortable && (
                                                        <span className="ml-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                            {getSortIcon(header.key as keyof RaidController)}
                                                        </span>
                                                    )}
                                                </div>
                                            </th>
                                ...
                                <tbody className="divide-y divide-gray-200">
                                    {currentItems.map((controller) => (
                                        <tr key={controller.id} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-sm font-medium text-gray-900">{controller.name}</td>
                                            <td className="px-6 py-4 text-sm text-gray-600">{controller.brand.name}</td>
                                            <td className="px-6 py-4 text-sm text-purple-600 font-medium">{controller.model}</td>
                                            <td className="px-6 py-4 text-sm text-gray-700 uppercase">{controller.supported_levels}</td>
                                            <td className="px-6 py-4 text-sm text-blue-600 font-medium">{controller.price} DH</td>
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex gap-3">
                                                    <Link
                                                        href={`/raid-controllers/${controller.id}/edit`}
                        ...
                        <div className="flex flex-col sm:flex-row justify-between items-center p-4 border-t border-gray-200">
                            <div className="mb-4 sm:mb-0 text-sm text-gray-600">
                                {sortedControllers.length} résultat{sortedControllers.length > 1 && 's'} • Page {currentPage} sur {totalPages}
                            </div>
