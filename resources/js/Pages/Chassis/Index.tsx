...
export default function Index({ chassis }: Props) {
    ...
    const [sortConfig, setSortConfig] = useState<{ key: keyof Chassis; direction: 'asc' | 'desc' } | null>(null);
    ...
    const handleDelete = (id: number) => {
        if (confirm("Voulez-vous vraiment supprimer ce châssis ?")) {
            destroy(`/chassis/${id}`, {
                preserveScroll: true,
                onSuccess: () => toast.success('Châssis supprimé avec succès'),
    ...
    const handleSort = (key: keyof Chassis) => {
    ...
    const filteredChassis = chassis.filter(chassisItem =>
        ...
        chassisItem.brand.name.toLowerCase().includes(searchTerm) ||
        chassisItem.type.toLowerCase().includes(searchTerm)
    );
    ...
    const currentItems = sortedChassis.slice(indexOfFirstItem, indexOfLastItem);
    const totalPages = Math.ceil(sortedChassis.length / itemsPerPage);

    const getSortIcon = (key: keyof Chassis) => {
                    ...
                    <div className="rounded-xl border border-gray-200 overflow-hidden shadow-sm">
                        <div className="overflow-x-auto">
                            <table className="w-full">
                                    ...
                                    <tr>
                                        {[
                                            ...
                                            <th 
                                                ...
                                                onClick={() => header.sortable && handleSort(header.key as keyof Chassis)}
                                            >
                                                <div className="flex items-center group">
                                                    {header.label}
                                                    {header.sortable && (
                                                        <span className="ml-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                            {getSortIcon(header.key as keyof Chassis)}
                                                        </span>
                                                    )}
                                                </div>
                                            </th>
                                ...
                                <tbody className="divide-y divide-gray-200">
                                    {currentItems.map((chassisItem) => (
                                        <tr key={chassisItem.id} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-sm font-medium text-gray-900">{chassisItem.name}</td>
                                            <td className="px-6 py-4 text-sm text-gray-600">{chassisItem.brand.name}</td>
                                            <td className="px-6 py-4 text-sm text-blue-600 font-medium">{chassisItem.price} DH</td>
                                            <td className="px-6 py-4 text-sm text-gray-600 uppercase font-medium">{chassisItem.type}</td>
                                            <td className="px-6 py-4 text-sm text-gray-700">{chassisItem.form_factor}</td>
                                            <td className="px-6 py-4 text-sm text-gray-700">{chassisItem.material}</td>
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex gap-3">
                                                    <Link
                                                        href={`/chassis/${chassisItem.id}/edit`}
                        ...
                        <div className="flex flex-col sm:flex-row justify-between items-center p-4 border-t border-gray-200">
                            <div className="mb-4 sm:mb-0 text-sm text-gray-600">
                                {sortedChassis.length} résultat{sortedChassis.length > 1 && 's'} • Page {currentPage} sur {totalPages}
                            </div>
