...
export default function Index({ graphicCards }: Props) {
    ...
    const [sortConfig, setSortConfig] = useState<{ key: keyof GraphicCard; direction: 'asc' | 'desc' } | null>(null);
    ...
    const handleDelete = (id: number) => {
        if (confirm("Voulez-vous vraiment supprimer cette carte graphique ?")) {
            destroy(`/graphic-cards/${id}`, {
                preserveScroll: true,
                onSuccess: () => toast.success('Carte graphique supprimée'),
    ...
    const handleSort = (key: keyof GraphicCard) => {
    ...
    const filteredGraphicCards = graphicCards.filter(card =>
        card.name.toLowerCase().includes(searchTerm) ||
        card.brand.name.toLowerCase().includes(searchTerm) ||
        card.gpu_architecture.toLowerCase().includes(searchTerm)
    );

    const sortedGraphicCards = [...filteredGraphicCards].sort((a, b) => {
    ...
    const currentItems = sortedGraphicCards.slice(indexOfFirstItem, indexOfLastItem);
    const totalPages = Math.ceil(sortedGraphicCards.length / itemsPerPage);

    const getSortIcon = (key: keyof GraphicCard) => {
                    ...
                    <div className="rounded-xl border border-gray-200 overflow-hidden shadow-sm">
                                ...
                                <thead className="bg-gray-50">
                                    <tr>
                                        {[
                                            ...
                                            { key: 'gpu_architecture', label: 'Architecture', sortable: true },
                                            { key: 'memory_type', label: 'Mémoire', sortable: true },
                                            ...
                                            <th 
                                                ...
                                                onClick={() => header.sortable && handleSort(header.key as keyof GraphicCard)}
                                            >
                                                <div className="flex items-center group">
                                                    {header.label}
                                                    {header.sortable && (
                                                        <span className="ml-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                            {getSortIcon(header.key as keyof GraphicCard)}
                                                        </span>
                                                    )}
                                                </div>
                                            </th>
                                ...
                                <tbody className="divide-y divide-gray-200">
                                    {currentItems.map((card) => (
                                        <tr key={card.id} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-sm text-gray-700">{card.gpu_architecture}</td>
                                            <td className="px-6 py-4 text-sm text-gray-700">{card.memory_type}</td>
                                            <td className="px-6 py-4 text-sm text-gray-700">{card.power_rating} W</td>
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex gap-3">
                                                    <Link
                                                        href={`/graphic-cards/${card.id}/edit`}
                        ...
                        <div className="flex flex-col sm:flex-row justify-between items-center p-4 border-t border-gray-200">
                            <div className="mb-4 sm:mb-0 text-sm text-gray-600">
                                {sortedGraphicCards.length} résultat{sortedGraphicCards.length > 1 && 's'} • Page {currentPage} sur {totalPages}
                            </div>
                        ...
                        {filteredGraphicCards.length === 0 && (
                            <div className="p-6 text-center text-gray-500 text-sm">
                                Aucune carte graphique trouvée
                            </div>
                        )}
                    </div>
                </div>
