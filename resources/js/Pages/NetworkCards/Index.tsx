...
export default function Index({ networkCards }: Props) {
    ...
    const handleDelete = (id: number) => {
        destroy(`/network-cards/${id}`, {
            preserveScroll: true,
            onSuccess: () => toast.success('Carte réseau supprimée avec succès'),
    ...
    const handleSort = (key: keyof NetworkCard) => {
    ...
    const filteredNetworkCards = networkCards.filter(card =>
        card.name.toLowerCase().includes(searchTerm) ||
        card.brand.name.toLowerCase().includes(searchTerm) ||
        card.interface.toLowerCase().includes(searchTerm)
    );

    const sortedNetworkCards = [...filteredNetworkCards].sort((a, b) => {
    ...
    const currentItems = sortedNetworkCards.slice(indexOfFirstItem, indexOfLastItem);
    const totalPages = Math.ceil(sortedNetworkCards.length / itemsPerPage);

    const getSortIcon = (key: keyof NetworkCard) => {
