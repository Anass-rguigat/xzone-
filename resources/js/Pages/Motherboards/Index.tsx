...
export default function Index({ motherboards }: Props) {
    ...
    const [sortConfig, setSortConfig] = useState<{ key: keyof Motherboard; direction: 'asc' | 'desc' } | null>(null);
    ...
    const handleDelete = (id: number) => {
        destroy(`/motherboards/${id}`, {
            preserveScroll: true,
            onSuccess: () => toast.success('Carte mère supprimée avec succès'),
    ...
    const handleSort = (key: keyof Motherboard) => {
    ...
    const filteredMotherboards = motherboards.filter(motherboard =>
        ...
        motherboard.brand.name.toLowerCase().includes(searchTerm) ||
        motherboard.cpu_socket.toLowerCase().includes(searchTerm)
    );
    ...
    const currentItems = sortedMotherboards.slice(indexOfFirstItem, indexOfLastItem);
    const totalPages = Math.ceil(sortedMotherboards.length / itemsPerPage);

    const getSortIcon = (key: keyof Motherboard) => {
                    ...
                    <div className="rounded-lg border border-gray-200 overflow-hidden shadow-sm">
                                ...
                                <thead className="bg-gray-50">
                                    <tr>
                                        {[
                                            ...
                                            { key: 'cpu_socket', label: 'Socket CPU', sortable: true },
                                            ...
                                            <th 
                                                ...
                                                onClick={() => header.sortable && handleSort(header.key as keyof Motherboard)}
                                            >
                                                <div className="flex items-center group">
                                                    {header.label}
                                                    {header.sortable && (
                                                        <span className="ml-1 opacity-50 group-hover:opacity-100 transition-opacity">
                                                            {getSortIcon(header.key as keyof Motherboard)}
                                                        </span>
                                                    )}
                                                </div>
                                            </th>
                                        ))}
                                        {(can(user, 'Delete_Composants') || can(user, 'Edit_Composants') || can(user, 'Show_Composants')) && (
                                            <th className="px-4 py-2 text-left text-xs font-semibold text-gray-700">Actions</th>
                                        )}
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-gray-200">
                                    {currentItems.map((motherboard) => (
                                        <tr key={motherboard.id} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="px-4 py-2 text-sm text-gray-900">{motherboard.name}</td>
                                            <td className="px-4 py-2 text-sm text-gray-600">{motherboard.brand.name}</td>
                                            <td className="px-4 py-2 text-sm text-gray-700">{motherboard.model}</td>
                                            <td className="px-4 py-2 text-sm text-blue-600 uppercase">{motherboard.cpu_socket}</td>
                                            <td className="px-4 py-2 text-sm text-gray-700">{motherboard.ram_slots}</td>
                                            <td className="px-4 py-2 text-sm text-gray-700">{motherboard.price} DH</td>
                                            ...
                                            <td className="px-4 py-2">
                                                <div className="flex gap-2">
                                                    {can(user, 'Show_Composants') && (
                                                        <Link
                                                            href={`/motherboards/${motherboard.id}`}
                        ...
                        <div className="flex flex-col sm:flex-row justify-between items-center p-3 border-t border-gray-200 text-xs">
                            <div className="mb-2 sm:mb-0 text-gray-600">
                                {sortedMotherboards.length} résultat{sortedMotherboards.length > 1 && 's'} • Page {currentPage} sur {totalPages}
                            </div>
...
// Les icônes restent identiques à celles du composant RAM
